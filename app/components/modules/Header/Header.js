  ...
  return (
    <main className='min-h-[100vh] max-[500px]:min-h-[200px]'>
        ...
        <ul className='flex justify-around p-2 text-white w-[250px]'>
                <li className='cursor-pointer' onClick={() => onLinkClick('home')}>Home</li>
                <li className='mx-2 cursor-pointer' onClick={() => onLinkClick('services')}>Services</li>
                <li className='cursor-pointer' onClick={() => onLinkClick('about')}>About us</li>
            </ul>
            <Image src={logo} width={300} height={300} alt='logo' className='w-[50px] h-[50px] rounded-[50%] z-10' />
        </div>


        <div className='top-0 right-0 fixed w-full justify-between backdrop-blur-lg m-auto items-center hidden max-[560px]:flex min-h-[100px]'>
      ...
      <div className={`fixed top-0 right-0 z-20 h-full ${styles.menuContainer} ${open ? styles.menuContainerOpen : ""}`}>
        <div className='w-[250px] h-full bg-[#1f1d3b] opacity-85 flex flex-col items-center'>
          ...
          <ul className='flex flex-col justify-center mx-auto items-center p-2 text-white w-[250px]'>
              <li onClick={() => { onLinkClick('home'); setOpen(false); }}>Home</li>
              <li className='my-8' onClick={() => { onLinkClick('services'); setOpen(false); }}>Services</li>
              <li onClick={() => { onLinkClick('about'); setOpen(false); }}>About us</li>
            </ul>
        </div>
